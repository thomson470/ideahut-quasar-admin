...
export default {
  ...
  created() {
    ...
    self.data = crud.get.object(params.data);
    self.form = form;
    self.fields = [];
    self.is_edit = params.is_edit;
    self.title =
      (crud.isString(form.title) && "" !== form.title
        ? form.title + " - "
        : "") + (self.is_edit ? self.$t("label.edit") : self.$t("label.new"));
    let fields = crud.get.array(form.fields);
    if (self.is_edit) {
      // edit
      self.id = crud.id.fromPk(self.form.id, self.data._pk_);
      for (const element of fields) {
        let field = crud.clone.field(element);
        field.value = crud.getFieldValue(field.name, self.data);
