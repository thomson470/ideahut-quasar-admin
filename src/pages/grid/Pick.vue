...
<template>
  <q-card
    :style="
      'min-width: 60vw; max-width: 90vw; max-height: 90vh;' + dialog.main.style
    "
  >
    <q-card-section
    ...
    <q-card-section style="max-height: 80vh" class="q-pa-xs q-pt-sm scroll">
      <q-table
        ...
        :rows-per-page-options="
          crud.isObject(pick.table) && pick.table.page?.options
            ? pick.table.page.options
            : [10, 20, 30]
        "
        @request="do_request"
        binary-state-sort
        :separator="crud.isObject(pick.table) ? pick.table.separator : 'cell'"
        bordered
      ...
      >
        ...
        <template v-slot:pagination="scope">
          ...
          >
            <template
              v-if="
                crud.isObject(pick.table) && crud.isObject(pick.table.page)
                  ? true === pick.table.page.count
                  : false
              "
              v-slot:append
            >
...
export default {
  ...
  created() {
    ...
    self.field = crud.get.object(params.field);
    self.relations = crud.get.array(params.relations);
    self.pick = crud.get.object(params.pick);
    self.pick._grid_id_ = self.template._grid_id_;
    self.search = {
      empty: true,
      filters: crud.copy(crud.get.array(self.pick.table.filters)),
    };
    self.table = {
      rows: [],
    ...
    on_close_dialog_search(filters) {
      if (crud.isArray(filters)) {
        ...
        for (const filter of search.filters) {
          v1 = crud.isDefined(filter.value) ? filter.value : "";
          v2 = crud.isDefined(filter.value2) ? filter.value2 : "";
          if ("" !== v1 || "" !== v2) {
            search.empty = false;
            break;
          }
