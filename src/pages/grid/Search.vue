...
export default {
  ...
  created() {
    ...
    self.filters = crud.isArray(params.filters) ? params.filters : [];
    self.template = crud.isObject(params.template) ? params.template : {};
    self.enums = {};
  ...
  methods: {
    ...
    on_filter_click() {
      for (const element of self.filters) {
        if (crud.isDefined(element.value) && element.value === null) {
          delete element.value;
        }
        if (crud.isDefined(element.value2) && element.value2 === null) {
          delete element.value2;
        }
      }
