<template>
    ...
    >
        <Keys
            :parameters="dialog.keys.parameters"
        />
    </q-dialog>
</template>
...
export default {
    components: {
        Keys: defineAsyncComponent(() => import("src/pages/CacheKeys")),
    },
    setup() {
        return {
            util,
            ...
            api.call({
                ...
                onSuccess(data, info) {
                    ...
                    if ("group" === info.type) {
                        ...
                        self.table.columns[1].format = (val) => { return 0 === val ? self.$t('label.unlimited') : val; };
                        self.table.columns[3].format = (val) => { return val && val > 0 ? val : self.$t('label.never_expire'); };
                        self.table.rows = [];
