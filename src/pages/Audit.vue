...
export default {
    components: {
        Search: defineAsyncComponent(() => import("src/pages/AuditSearch")),
            ...
            columns: [
                {
                    ...
                    format: function(val, row) {
                        return util.format.date(util.getFieldValue("entry", row), {format: "YYYY-MM-DD HH:mm:ss"});
                    },
                },
                {
                    name: "id",
                            ...
                            for (const field of self.info.fields) {
                                ...
                                if (true === self.info.isSoftDelete && "deletedOn" === field.name) {
                                    ...
                                    column.format = function(val) {
                                        try { return util.format.date(util.getFieldValue("deletedOn", val), {format: dateFormat}) } catch(e) { return val; };
                                    };
                                }
                                    ...
                                    if ("createdOn" === field.name) {
                                        ...
                                        column.pattern = util.isString(field.format) ? field.format : dateFormat;
                                        column.format = function(val) {
                                            try { return util.format.date(util.getFieldValue("createdOn", val), {format: dateFormat}) } catch(e) { return val; };
                                        };
                                    }
                                        ...
                                        column.pattern = util.isString(field.format) ? field.format : dateFormat;
                                        column.format = function(val) {
                                            try { return util.format.date(util.getFieldValue("updatedOn", val), {format: dateFormat}) } catch(e) { return val; };
                                        };
