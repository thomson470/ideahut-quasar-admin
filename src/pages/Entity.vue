<template>
    <div class="row q-mt-none q-pa-sm">
        <div class="col-md-6 col-xs-12 q-pa-xs q-mb-sm">
            ...
            >
                <template v-slot:top-right>
                    <q-input borderless dense debounce="300" v-model="setting.filter" :placeholder="$t('label.search')">
        ...
        <div class="col-md-6 col-xs-12 q-pa-xs q-mb-sm">
            ...
            >
                <template v-slot:top-right>
                    <q-input borderless dense debounce="300" v-model="metadata.filter" :placeholder="$t('label.search')">
                        <template v-slot:append>
                            <q-icon name="search" />
                        </template>
                    </q-input>
                            ...
                            <div class="text-left q-pa-sm">
                                ...
                                >
                                    <template v-slot:top>
                                        ...
                                        <span class="q-ml-md">[ {{ $t('label.type') }}: {{ props.row.id.type }}{{ util.isString(props.row.id.name) ? (', ' + $t('label.field') + ': ' + props.row.id.name) : '' }} ]</span>
                                    </template>
                                </q-table>
                                <q-table
                                    class="table-child q-mt-sm"
                                    ...
                                    <template v-slot:body="props">
                                        <q-tr :props="props">
                                            <q-td
                                                ...
                                                :key="col.name"
                                                :props="props"
                                                :style="'type' === col.name && true === props.row.isEntity ? 'cursor: pointer;' : ''"
                                                @click="'type' === col.name && true === props.row.isEntity ? on_dialog_entity_show(props.row.type) : null"
                                            >
                                                {{ col.value }}
                                            </q-td>
                                        </q-tr>
        ...
        */
         ...
         */
         on_entity_grid_copy_to_clipboard_click() {
            ...
            setTimeout(function() { self.dialog.grid.copied = false; }, 3000);
         },
