...
<template>
  <div class="row q-mt-none q-pa-sm">
    <div
      :class="(metadata?.rows?.length ? 'col-md-6 ' : 'col-md-12 ') + ' col-xs-12 q-pa-xs q-mb-sm'"
    >
      <q-table
        ...
        <template v-slot:body="props">
          <q-tr :props="props">
            ...
            <q-td
              ...
              @click="'type' === col.name ? on_dialog_entity_show(props.row.type) : null"
            >
              {{ col.value }}
            </q-td>
          </q-tr>
            ...
            <q-td colspan="100%">
              <div class="text-left q-pa-sm">
                <div>
                  ...
                  >
                    <q-list>
                      <q-item clickable v-close-popup @click="on_entity_grid_click(props, 'json')">
                        <q-item-section>JSON</q-item-section>
                      </q-item>
                      <q-separator />
                      <q-item clickable v-close-popup @click="on_entity_grid_click(props, 'yaml')">
                ...
                >
                  <template v-slot:top>
                    <span class="text-weight-bolder">{{ $t('label.identifier') }}</span>
                    <span class="q-ml-md"
                ...
                >
                  <template v-slot:top>
                    <span class="text-weight-bolder">{{ $t('label.field') }}</span>
                  </template>
                  <template v-slot:body="props">
                    <q-tr :props="props">
                      <q-td
                ...
                >
                  <template v-slot:top>
                    <span class="text-weight-bolder">{{ $t('label.transient') }}</span>
                  </template>
                  <template v-slot:body="props">
                    <q-tr :props="props">
                      <q-td
  ...
  <!--
      DIALOG REPLICA
    -->
  <q-dialog v-model="dialog.replica.show" persistent backdrop-filter="blur(1px)">
    <q-card
      :style="
        ($q.screen.lt.md ? 'width: 80vw; max-width: 81vw;' : 'width: 60vw; max-width: 61vw;') +
        dialog.replica.style
      "
    >
      <q-card-section
        class="q-pa-none header-main"
      ...
      <q-card-section
        :style="
          $q.screen.lt.md ? 'max-height: 65vh;' : 'max-height: 70vh; width: 60vw; max-width: 61vw;'
        "
        class="q-pa-xs q-mt-xs scroll"
      >
        <q-list>
          <q-item v-for="(sql, index) in dialog.replica.sqls" :key="index" class="q-pa-none">
            <q-input
              type="text"
...
export default {
  ...
  methods: {
    ...
    get_entity_pagination(props) {
      let pagination = props?.pagination ? props.pagination : self.entity.pagination
      if (pagination) {
    ...
    on_entity_filter_click() {
      ...
      if (!(util.isString(filters.isAuditEnabled) && '' !== filters.isAuditEnabled)) {
